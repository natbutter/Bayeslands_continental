...
class BayesLands():
    ...
    def run_badlands(self, input_vector):
        ...
        if init == True:
            ...
            filename='init_topo_polygon/Paleotopo_P100_50km_prec2_'+ str(int(self.ID)) +'.csv' 
    ...
    def likelihood_func(self,input_vector): 
        ...
        avg_rmse_ocean = np.average(rmse_ocean)
        ...
        print('RMSE :--: Elev ', rmse_elev, 'Erdp', rmse_erodep, 'Ocean', avg_rmse_ocean)
 
        return [likelihood, pred_elev_vec, pred_erodep_pts_vec, likelihood, rmse_elev_pts, rmse_erodep, rmse_ocean, avg_rmse_ocean ]
    ...
    def likelihoodSurface(self):
        ...
        font = 9
        width = 1
        xml_files = ['AUSP1306.xml', 'AUSP1307.xml','AUSP1308.xml','AUSP1309.xml','AUSP1310.xml', 'AUSP1311.xml', 'AUSP1312.xml']

        variables = np.zeros((self.vec_parameters.size,samples))
        pos_rmse = np.zeros((len(xml_files)))
        pos_likl = np.zeros((len(xml_files)))
        pos_rmse_elev = np.zeros((len(xml_files)))
        pos_rmse_erodep = np.zeros((len(xml_files)))
        pos_avg_rmse_ocean = np.zeros((len(xml_files)))
        pos_rmse_total = np.zeros((len(xml_files)))

        print ('variables', variables.shape)

        for i in xml_files:
            [likelihood_proposal, predicted_elev, pred_erodep_pts, likl_without_temp, avg_rmse_el, avg_rmse_er, rmse_ocean, avg_rmse_ocean] = self.likelihood_func(self.vec_parameters)
            ...
            pos_likl[i]=likelihood_proposal  # contains  all proposal liklihood (accepted and rejected ones)        
            pos_rmse_elev[i] = avg_rmse_el
            pos_rmse_erodep[i] = avg_rmse_er
            pos_avg_rmse_ocean[i] = avg_rmse_ocean
...
def main():
    ...
    samples = 1 #input("Please enter number of samples (Make sure it is a perfect square): \n")
    ...
    minlimits_others = [1.e-6, 0, 0, 0,  0 , 0 , 0 , 0, 23001, 4, 0 ]  # used for Bayeslands environmental params  (stage 2)
    maxlimits_others = [2.e-6, 1, 2, 0.2, 0.2, 0.2, 1, 10, 25002, 6, 0.2]
