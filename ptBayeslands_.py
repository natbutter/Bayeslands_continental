...
class ptReplica(multiprocessing.Process):
    ...
    def edit_DBF(self, inittopo_vec):  #edit shape file for init topo reconstruction 
        ...
        for i,rec in enumerate(db):
            ...
            #     rec["ELEVATION"] = (inittopo_vec[3]*20) + expert_know[i]
            #     rec.store()
            #     del rec
            # elif rec[0] == "Marine abyssal":
            #     rec["ELEVATION"] = (inittopo_vec[5]*10) + expert_know[i]
            #     rec.store()
            #     del rec
            # elif rec[0] == "Fluvial":
            #     rec["ELEVATION"] = (inittopo_vec[6]*10) + expert_know[i]
            #     rec.store()
            #     del rec
            # elif rec[0] == "Coastal depositional paralic":
            #     rec["ELEVATION"] = (inittopo_vec[7]*200) + expert_know[i]
            #     rec.store()
            #     del rec
            # elif rec[0] == "Marine very shallow":
            #     rec["ELEVATION"] = (inittopo_vec[8]*10) + expert_know[i]
            #     rec.store()
            #     del rec
            # elif rec[0] == "Fluvial-lacustrine":
            #     rec["ELEVATION"] = (inittopo_vec[9]*10) + expert_know[i]
            #     rec.store()
            #     del rec
            # elif rec[0] == "Lacustrine":
            #     rec["ELEVATION"] = (inittopo_vec[10]*10) + expert_know[i]
            #     rec.store()
            #     del rec
            else:
                pass
